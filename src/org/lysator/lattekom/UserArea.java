...
public class UserArea {
    ...
    Map<String, Hollerith> blocks = new HashMap<String, Hollerith>();
    String charset;
    int textNo;

    protected UserArea(Text text) throws IllegalArgumentException {
    ...
    public String[] getBlockNames() {
        Set<String> nameSet = blocks.keySet();
        String[] names = new String[nameSet.size()];
        int i = 0;
        for (Iterator<String> setIter = nameSet.iterator(); setIter.hasNext(); i++) {
    ...
    private void parse(Text text) throws IllegalArgumentException {
    ...
    public byte[] toNetwork() {
        try {
            ...
            List<Hollerith> blockData = new LinkedList<Hollerith>();
            ByteArrayOutputStream tocStream = new ByteArrayOutputStream();
            for (Iterator<Entry<String, Hollerith>> i = blocks.entrySet()
                    .iterator(); i.hasNext();) {
                Entry<String, Hollerith> entry = i.next();
                Hollerith tocEntry = new Hollerith(
                        ((String) entry.getKey()).getBytes(charset), charset);
    ...
    private void parse(byte[] data, String charset)
        ...
        try {
            ...
            Hollerith toc = (Hollerith) KomTokenReader.readToken(
                    userAreaStream, charset);
            ByteArrayInputStream tocStream = new ByteArrayInputStream(
                    toc.getContents());
            List<String> blockNames = new LinkedList<String>();
            Hollerith nextBlock = (Hollerith) KomTokenReader.readToken(
                    tocStream, charset);
            while (nextBlock != null) {
                ...
                nextBlock = (Hollerith) KomTokenReader.readToken(tocStream,
                        charset);
            }

            for (Iterator<String> names = blockNames.iterator(); names
                    .hasNext();) {
                String name = (String) names.next();
                Hollerith blockData = (Hollerith) KomTokenReader.readToken(
                        userAreaStream, charset);
                blocks.put(name, blockData);
                if (Debug.ENABLED) {
                    Debug.println("User-Area block \"" + name
                            + "\" parsed with "
